...
Для проверки работы необходимо предоставить **ссылку на публичный репозиторий GitHub** и **загрузить электронный отчет (.pdf/.docx) в LMS**.

Репозиторий должен содержать:
1.  **`README.md`** — файл с описанием вашего варианта задания, инструкциями по запуску Docker-окружения и краткими выводами по результатам анализа.
2.  **Файл DAG (`.py`)** — Python-скрипт с реализацией ETL-конвейера для Apache Airflow.
3.  **Исходный код проекта** — все файлы, необходимые для воспроизводимости решения (`docker-compose.yml`, `requirements.txt`, скрипты для проверки и очистки).
4.  **Примеры результатов** — примеры сгенерированных отчетов (`.txt`, `.csv`), которые отправляются по email.
...
*   **Корректность реализации DAG и ETL-логики (40%)**
    ...
    *   Задачи выполняются параллельно там, где это возможно.
    *   ETL-логика (консолидация, расчет метрик) реализована верно в соответствии с вариантом.
    *   Процесс идемпотентен (повторные запуски не приводят к дублированию данных).

*   **Воспроизводимость и использование Docker (20%)**
    *   Проект успешно запускается одной командой `docker compose up -d`.
...
*   **Функциональность и бизнес-ценность (20%)**
    ...
    *   Email-уведомление успешно формируется и отправляется в MailHog с корректными вложениями.

*   **Качество документации и отчета (20%)**
    *   `README.md` содержит четкие инструкции по запуску и проверке.
    *   Электронный отчет оформлен согласно требованиям, содержит описание, ключевые фрагменты кода, выводы и ссылку на репозиторий.

---
**Качественная оценка:**
*   **5 (Отлично):** Все критерии выполнены. DAG работает стабильно, код хорошо структурирован, отчет и `README` исчерпывающе описывают решение.
*   **4 (Хорошо):** Решение полностью функционально, но есть незначительные недочеты (например, неоптимальная структура DAG, неполные комментарии в коде).
*   **3 (Удовлетворительно):** Основная функциональность реализована, но присутствуют ошибки или недоработки (например, DAG периодически падает, отчет неполный).
*   **2 (Неудовлетворительно):** Проект не запускается, ETL-логика неверна, задание не выполнено.
