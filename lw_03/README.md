...
2. **Сборка и запуск всех сервисов**:
   ```bash
   # Сборка кастомного образа с зависимостями
...
| `Dockerfile` | Кастомный образ Airflow с зависимостями | 🔧 Решает проблему openpyxl |
...
| `requirements.txt` | Зависимости Python (pandas, openpyxl) | 📦 Для Docker образа |
...
**Решение**: Используйте кастомный Docker образ с предустановленными зависимостями:
```bash
# Остановите существующие контейнеры
docker compose down

# Соберите кастомный образ с зависимостями
docker build -t airflow-custom:latest .
