...
### Пример выполнения задания №30. Мобильные приложения - Коэффициент удержания
...
*   Клонируйте репозиторий с заданием в домашний каталог ВМ:
    ```bash
...
*   Перейдите в каталог проекта (`cd DCCAS/lw_03`) и запустите Docker-контейнеры с помощью `sudo docker compose up -d`.
*   Откройте веб-интерфейс Airflow (`http://localhost:8080`).
*   **Задача:** Изучите и опишите в отчете основные элементы интерфейса Apache Airflow (страница DAGs, Grid View, Graph View, Logs).

### 3. Проектирование архитектуры

*   **Задача:** Спроектируйте верхнеуровневую архитектуру для вашего варианта задания в [draw.io](https://app.diagrams.net/) или аналогичном инструменте.
*   Архитектура должна включать следующие слои:
    *   **Source Layer** (слой источников данных: CSV, XLSX, JSON).
    *   **Storage Layer** (слой хранения: например, SQLite или PostgreSQL).
    *   **Business Layer** (слой доступа для бизнес-пользователей: отчеты, email-уведомления).

### 4. Реализация ETL-процесса

*   Реализуйте ETL-конвейер в виде DAG для Apache Airflow в соответствии с вашим вариантом задания.
...
Варианты заданий для выполнения лабораторной работы доступны по ссылке:
**[Скачать варианты заданий](http://95.131.149.21/moodle/pluginfile.php/3453/mod_assign/introattachment/0/LW_3new.pdf?forcedownload=1)**

### 📁 Материалы для оценки

Для проверки работы необходимо:
1.  **Загрузить электронный отчет** (`.pdf` или `.docx`) в LMS.
2.  Предоставить **ссылку на публичный репозиторий GitHub**, содержащий:
    *   **`README.md`** — файл с описанием вашего варианта задания, инструкциями по запуску и выводами.
    *   **Файл DAG (`.py`)** — Python-скрипт с реализацией ETL-конвейера.
    *   **Исходный код проекта** — все файлы для воспроизводимости (`docker-compose.yml`, `requirements.txt` и др.).
    *   **Примеры результатов** — сгенерированные отчеты (`.txt`, `.csv`).

### 📊 Критерии оценки

Итоговая оценка формируется на основе следующих критериев:

*   **Корректность реализации DAG и ETL-логики (40%)**
    *   DAG корректно описывает последовательность задач (Extract, Transform, Load, Report, Notify).
    *   Задачи выполняются параллельно, где это возможно.
    *   ETL-логика (консолидация, расчет метрик) реализована верно.
    *   Процесс идемпотентен (повторные запуски не приводят к дублированию данных).

*   **Воспроизводимость и использование Docker (20%)**
    *   Проект успешно запускается командой `docker compose up -d`.
    *   Все сервисы (Airflow, Postgres, MailHog) корректно взаимодействуют.
    *   Структура проекта логична.

*   **Функциональность и бизнес-ценность (20%)**
    *   DAG успешно выполняется по расписанию или при ручном запуске.
    *   Результаты анализа корректны и соответствуют бизнес-задаче.
    *   Email-уведомление успешно формируется и отправляется в MailHog с вложениями.

*   **Качество документации и отчета (20%)**
    *   `README.md` содержит четкие инструкции по запуску и проверке.
    *   Электронный отчет оформлен согласно требованиям.

---

### ⭐ Пример выполнения. Задание №30

#### Бизнес-задача. Анализ удержания пользователей мобильных приложений

*   **Описание.** Компания-разработчик нуждается в автоматизированном анализе Retention Rate (коэффициента удержания), чтобы оценить, какой процент пользователей продолжает использовать приложение после установки.

*   **Бизнес-ценность:**
    *   📊 **Автоматизация аналитики.** Ежедневный расчет ключевых метрик.
    *   🎯 **Выявление проблемных категорий.** Быстрое определение приложений с низким удержанием.
    *   📈 **Принятие решений на основе данных.** Объективная оценка успешности продуктов.
    *   ⏰ **Экономия времени.** Автоматическая обработка данных.
    *   📧 **Своевременное уведомление.** Автоматическая отправка отчетов.

*   **Исходные данные:**
    1.  `apps.csv`. Справочник приложений (`app_id`, `category`).
    2.  `installs.xlsx`. Статистика установок (`app_id`, `installs_count`).
    3.  `uninstalls.json`. Статистика удалений (`app_id`, `uninstalls_count`).
...
*   **Ожидаемый результат:** Ежедневный аналитический отчет по категориям приложений с рекомендациями, отправляемый на email.
